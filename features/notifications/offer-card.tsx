...
export function OfferCard({ offer, onAccept, onReject }: OfferCardProps) {
  ...
  const getTitle = () => {
    if (isJob) return offer.details.businessName
    if (isPartnership) return offer.details.businessName
    if (isShareSale) return offer.details.businessName
    return 'Новое предложение'
  }

  const getSubtitle = () => {
    if (isJob) return `Вакансия: ${offer.details.role}`
  ...
  return (
    <>
      <button
        className="bg-zinc-900/90 border border-white/10 rounded-xl overflow-hidden backdrop-blur-md shadow-xl w-full max-w-sm animate-in slide-in-from-bottom-5 fade-in duration-300 cursor-pointer hover:bg-zinc-800/90 transition-colors group text-left"
        onClick={() => {
        ...
        <div className="p-4 flex items-center gap-4">
          <div
            className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
              isJob ? 'bg-blue-500/20' : isPartnership ? 'bg-purple-500/20' : 'bg-green-500/20'
            }`}
          >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start mb-1">
              ...
              <Badge
                className="text-[10px] px-1.5 h-5 border-white/20 text-white/60 shrink-0"
                variant="outline"
              >
