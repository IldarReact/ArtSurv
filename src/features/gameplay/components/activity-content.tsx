...
import { FamilyActivity } from '@/src/features/activities/family/family-activity'
import { InvestmentsActivity } from '@/src/features/activities/investments/investments-activity'
import { RelocationActivity } from '@/src/features/activities/relocation/relocation-activity'
...
export function ActivityContent(): React.JSX.Element | null {
  ...
  // Multiplayer state
  const [turnReadyStats, setTurnReadyStats] = useState({ allReady: false, ready: 0, total: 1 })
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToTurnReadyStatus((ready, total, allReady) => {
      setTurnReadyStats({ allReady, ready, total })
  ...
  const currentActivity = activeActivity ?? 'family'
  ...
  return (
    <div className="relative min-h-screen pt-24">
      <div
        ...
        style={{
          backgroundImage: backgroundImages[currentActivity],
          zIndex: 0,
        }}
      />
