  ...
  const openingProgress =
    openingQuarters > 0
      ? {
          ...
          quartersLeft: normalizeDurationFromTurns(openingQuarters),
          remainingDuration: normalizeDurationFromTurns(openingQuarters),
          title: `Открытие: ${name}`,
          totalCost,
          totalDuration: normalizeDurationFromTurns(openingQuarters),
          totalQuarters: normalizeDurationFromTurns(openingQuarters),
          upfrontCost,
        }
      : undefined
...
export function createBusinessBranch(
  ...
  const baseDurationMonths = mainBusiness.openingProgress?.totalDuration ?? MONTHS_PER_QUARTER
  const branchDurationMonths = normalizeDurationMonths(
    Math.max(MONTHS_PER_QUARTER, Math.round(baseDurationMonths * BRANCH_OPENING_TIME_MULTIPLIER)),
  )
  ...
  return {
    ...
    openingProgress: {
      ...
      investedAmount: cost,
      quartersLeft: branchDurationMonths,
      remainingDuration: branchDurationMonths,
      title: `Открытие филиала: ${branchName}`,
      totalCost: cost,
      totalDuration: branchDurationMonths,
      totalQuarters: branchDurationMonths,
      upfrontCost: cost,
    },
    playerRoles: {
      managerialRoles: [],
