...
describe('Education Processor', () => {
  ...
  it('should progress active courses', () => {
    const activeCourses = [{ ...mockCourse, remainingDuration: 6 }]

    const result = processEducation(activeCourses, [], [mockSkill], 1, 2025)

    expect(result.activeCourses[0].remainingDuration).toBe(3)
    expect(result.notifications).toHaveLength(0)
  })

  it('should complete course and increase skill level', () => {
    const activeCourses = [{ ...mockCourse, remainingDuration: 3 }]

    const result = processEducation(activeCourses, [], [mockSkill], 1, 2025)

    expect(result.activeCourses).toHaveLength(0)
    expect(result.updatedSkills.find((s) => s.name === 'Marketing')?.level).toBe(3)
  ...
  it('should create new skill upon course completion if not exists', () => {
    const activeCourses = [{ ...mockCourse, remainingDuration: 3, skillName: 'Design' }]
  ...
  it('should progress university studies', () => {
    ...
    expect(result.activeUniversity[0].remainingDuration).toBe(3)
  })
})
