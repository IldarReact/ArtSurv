...
describe('Centralized Stats Logic', () => {
  ...
  it('applyStatChangeEffect applies one_time and ignores temporary', () => {
    store.applyStatChangeEffect({
      effects: { energy: -10, money: 50 },
      kind: 'one_time',
    })

    expect(state.player.stats.energy).toBe(90)
    expect(state.player.stats.money).toBe(1050)
