...
export function handleFireEmployee(
  ...
  const employeeToFire = business.employees.find((e) => e.id === employeeId)
  if (!employeeToFire) {
    state.pushNotification({
      message: 'Сотрудник не найден в этом бизнесе.',
  ...
  state.pushNotification({
    message: `${employeeToFire.name} уволен из ${business.name}.`,
