...
import { CardHeader, CardTitle, CardDescription } from '@/shared/components/card'
    ...
    <div className="flex items-start justify-between gap-3">
            ...
            {!isVacancy && stars > 0 && (
              <div className="flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => {
                  const starKey = `star-${String(i)}-${name}`
                  return (
                    <Star
                      className={cn(
      ...
      {salary !== undefined && (
        <div className="text-right flex flex-col items-end">
          <div
            className="text-2xl font-black text-green-400 flex items-center justify-end drop-shadow-[0_0_12px_rgba(74,222,128,0.4)] tracking-tighter"
